...
export default class ProductForm {
  ...
  async render() {
    const fetchCategories = fetchJson(`${BACKEND_URL}/api/rest/categories?_sort=weight&_refs=subcategory`);
    let fetchProduct;
    if (this.productId) {
      fetchProduct = fetchJson(`${BACKEND_URL}/api/rest/products?id=${this.productId}`)
    } else {
      fetchProduct = new Promise((resolve) => resolve (this.defaultFormData))
    }
    const [categories, products] = await Promise.all([fetchCategories, fetchProduct]);
